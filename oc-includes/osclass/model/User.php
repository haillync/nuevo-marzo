 ...
 */
    ...
    class User extends DAO
    {
         ...
         * @return array
         */
        public function findCompanyByUsername($username, $locale = null)
        {
			$sql = "SELECT s_name FROM oc_t_user WHERE s_name= '".$username."' AND b_company=1";
         ...
         * @return array
         */
        public function findVipByUsername($username, $locale = null)
        {
            $sql = "SELECT s_name FROM oc_t_user WHERE s_name= '".$username."' AND b_vip=1";
            $result=$this->dao->query($sql);
...
/**
         ...
         * @return array
         */
        private function insertRubro($userId, $rubroId)
        {
            $array_set = array(
                'fk_i_user_id'      => $userId,
                'fk_i_rubro_id'     => $rubroId
            );

            return $this->dao->insert(DB_TABLE_PREFIX.'t_user_rubro', $array_set);
        }
         ...
         * @return bool
         */
        public function updateRubro($idRubro,$idUser )
        {
            $conditions = array('fk_i_user_id' => $idUser);
            $exist = $this->existRubro($conditions);

            if(!$exist) {
                $result = $this->insertRubro($idUser, $idRubro);
                return $result;
            }
            ...
            return $this->dao->update(DB_TABLE_PREFIX.'t_user_rubro', array('fk_i_rubro_id' => $idRubro), $array_where);
        }
         ...
         * @return bool
         */
        private function existRubro($conditions)
        {
            $this->dao->select();
            $this->dao->from(DB_TABLE_PREFIX.'t_user_rubro');
            $this->dao->where($conditions);
        ...
        function lastAccess($userId, $date, $ip, $time = NULL) {
				$geopluginURL='http://api.ipinfodb.com/v3/ip-city/?key=55f22a71cfc95558ccec23d9dd545be39df392f7638fa6100a3026a593a3bf85&format=json&ip='.$ip;
				$unarr= file_get_contents($geopluginURL);     // Get File Contents
				$datos = json_decode($unarr,true);
				
				$cityName = $datos["cityName"];  // City Name
				
				$regionName= $datos["regionName"]; // Region Name

				$countryName = $datos["countryName"]; // Country Name

				$country_code = $datos["countryCode"]; // Country Code

				$latitud = $datos["latitude"]; // Latitud

				$longitud = $datos["longitude"]; // Longitud
				//echo "Pais: ".$datos["countryName"].", Codigo de Pais:".$datos["countryCode"].", Region: ".$datos["regionName"].", Ciudad: ".$datos["cityName"].", Latitud: ".$datos["latitude"].", Longitud: ".$datos["longitude"];
			...
			$user = $this->findByPrimaryKey($userId);
			if($regionName==""){				
				$regionName= $user['s_region']; 
				$fk_i_region_id = $user['fk_i_region_id'];
			}else{
				
				$region= Region::newInstance()->findByName($regionName, $country_code);
				$fk_i_region_id= $region['pk_i_id'];
			}
			
            return $this->update(array('dt_access_date' => $date, 's_access_ip' => $ip, 'fk_c_country_code' => $country_code, 's_country' => $countryName, 'fk_i_region_id' => $fk_i_region_id, 's_region' => $regionName, 's_city' => $cityName , 'd_coord_lat' => $latitud, 'd_coord_long'=> $longitud), array('pk_i_id' => $userId));
        }
		   /**
         * Insert last access data
         *
        ...
        function insertCompany($userId, $company) {
            ...
            return $this->update(array('b_company' => $company), array('pk_i_id' => $userId));
        }
		...
		public function numItems($idUser)
		{
			$this->dao->select("i_items");
            $this->dao->from(DB_TABLE_PREFIX.'t_user');
            $this->dao->where("pk_i_id=".$idUser);
		...
		public function tipoUsuario($idUser)
		{
			$this->dao->select("s_tipo");
            $this->dao->from(DB_TABLE_PREFIX.'t_user');
            $this->dao->where("pk_i_id=".$idUser);
