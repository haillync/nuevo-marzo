 ...
 */
    ...
    Class UserActions
    {
        ...
        function add()
        {
			...
			if ($userId==''){
				osc_add_flash_error_message(_m("No inserto"));
				BaseModel::redirectTo( osc_register_account_url() );
			}
			...
			$rubro=Params::getParam('rubroId');
			if (Params::getParam('s_tipo')=='profesional'){
				if ( $rubro=='') {
					osc_add_flash_error_message(_m("No inserto Rubro"));
					BaseModel::redirectTo( osc_register_account_url() );
				}else{                
					$this->manager->updateRubro($rubro, $userId);                
				}
			}
			...
			$this->uploadUserResources( $input['photos'] , $userId);
        ...
        function prepareData($is_add)
        {
            ...
            if ( $is_add ) {
				...
				$b_company=User::newInstance()->findCompanyByUsername($username);
				$rubro=UserRubro::newInstance()->findRubroByUserId($userID);
				if (!$b_company){
					if ($rubro){
						$input['s_tipo'] = 'profesional';
					}else{
						$input['s_tipo'] = 'usuario';
					}					
				}else{
					$input['s_tipo'] = 'ilimitado';
				}				
            }
            ...
            if( intval( Params::getParam('cityId') ) ) {
			 ...
			 $input['s_access_ip'] = get_ip();		
			
			$input['s_tipo']         = 'usuario';//Params::getParam('s_tipo');
			//if ($input['s_tipo']=='usuario'){
				...
				$country_code = $datos["countryCode"]; // Country Code latitude
			...
			/*}else
				...
				$coordenadas= obtener_coordenada($countryName, $regionName, $cityName, $input['s_address']);
           ...
           // $input['s_city_area']    = Params::getParam('cityArea');
            ...
            $input['d_coord_lat']    = (Params::getParam('d_coord_lat')  != '') ? Params::getParam('d_coord_lat') : $latitud;
            $input['d_coord_long']   = (Params::getParam('d_coord_long') != '') ? Params::getParam('d_coord_long') : $longitud;
            //$input['b_company']      = (Params::getParam('b_company') != '' && Params::getParam('b_company') != 0) ? 1 : 0;		
			//$input['s_licencia']     = Params::getParam('licencia');
			//$input['s_matri']        = Params::getParam('matricula');
	...
	private function checkSize($aResources)
        ...
        public function uploadUserResources($aResources,$userId)
        {
            if($aResources != '') {
                //$itemResourceManager = ItemResource::newInstance();
                $folder = osc_uploads_path().(floor($userId/100))."/";

                $numImagesUser = osc_max_images_per_user();//$numImagesItems = osc_max_images_per_item();
                $numImages = $userResourceManager->countResources($userId);
                foreach ($aResources['error'] as $key => $error) {
                    if($numImagesUser==0 || ($numImagesUser>0 && $numImages<$numImagesUser)) {
                        if ($error == UPLOAD_ERR_OK) {
		...
		function obtener_coordenada($pais,$provincia,$ciudad, $calle){
			$direccion = $calle;
			$direccion .=",".$ciudad;
			$direccion .=",".$provincia;
			$direccion .=",".$pais;
			$resultado = json_decode(file_get_contents(sprintf('http://maps.googleapis.com/maps/api/geocode/json?sensor=false&address=%s', urlencode($direccion))));
